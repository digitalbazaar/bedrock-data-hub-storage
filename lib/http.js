...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      await _authorize(
        {dataHubId, req, expectedTarget, expectedAction: 'write'});
...
// TODO: consider updating ocapld to accept multiple expectedTargets ...
// ... to allow for invocation of a zcap on either a specific item in
// a collection or on the collection itself; this may make "autogeneration"
// of a zcap for a data hub document less awkward; instead the client would
// invoke the collection URL as the zcap ... and require the collection
// to have `@context` security-v2?
...
async function _authorize({dataHubId, req, expectedTarget, expectedAction}) {
  ...
  const wrappedDocumentLoader = async url => {
    // TODO: change to if url matches `dataHubId` (but this will require
    // changing clients to use it and another change to allow `expectedTarget`
    // to be an array of options, i.e. [<collectionId>, <collectionItemId>]
  ...
  const getInvokedCapability = async ({id, expectedTarget}) => {
    // we only need to dereference the invoked capability if it doesn't match
    // the expected target (i.e., it is not the root capability) as this zcap
    // is automatically authorized and will be retrieved via the document loader
    if(id === expectedTarget) {
      const {document} = await wrappedDocumentLoader(id);
      return document;
    }
    return _getInvokedCapability({id, expectedTarget});
  };

  const url = `${config.server.baseUri}${req.originalUrl}`;
  const {method, headers} = req;
  const result = verifyCapabilityInvocation({
    url, method, headers,
    getInvokedCapability,
    documentLoader: wrappedDocumentLoader,
    expectedHost: config.server.host,
    expectedTarget, expectedAction,
...
async function _getInvokedCapability({id, expectedTarget}) {
  try {
    const {authorization} = await brZCapStorage.authorizations.get({
      id,
