...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      const result = _getInvocationTarget(id);
      if(!result) {
      ...
      const zcap = await _generateRootCapability({id, target, dataHubId});
      res.json(zcap);
    }));
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      await _authorize({
        dataHubId, req, expectedTarget, expectedRootCapability,
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const dataHubId = _getDataHubId(req.params.dataHubId);
      const docPath = `/documents/${encodeURIComponent(id)}`;
  ...
  const documentLoader = extendContextLoader(async url => {
    ...
    if(result) {
      ...
      return {
        contextUrl: null,
        documentUrl: url,
        document: await _generateRootCapability({id: url, target, config})
      };
    }
...
async function _authorize({
  dataHubId, req, expectedTarget, expectedRootCapability, expectedAction
}) {
  ...
  const wrappedDocumentLoader = async url => {
    ...
    if(result) {
      ...
      return {
        contextUrl: null,
        documentUrl: url,
        document: await _generateRootCapability({id: url, target, dataHubId})
      };
    }
    return DOCUMENT_LOADER(url);
  };
...
async function getInvokedCapability({id, expectedTarget}) {
  ...
  if(result) {
    // dynamically generate zcap for root capability
    const {target, dataHubId} = result;
    return _generateRootCapability({id, target, dataHubId});
  }
...
async function _generateRootCapability({id, target, dataHubId, config = null}) {
  if(!config) {
    ({config} = await storage.getConfig({actor: null, id: dataHubId}));
  }
  // dynamically generate zcap for root capability
  return {
    '@context': SECURITY_CONTEXT_V2_URL,
