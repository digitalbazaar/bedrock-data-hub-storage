...
const {CapabilityDelegation, CapabilityInvocation} = require('ocapld');
...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const storer = invoker;
      const capability = req.body;
      await _verifyDelegation({dataHubId, storer, capability});
      await brZCapStorage.delegated.insert({storer, capability});
      res.status(204).end();
    }));
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      const {id} = req.query;
      if(id) {
        const {capability} = await brZCapStorage.delegated.get(
          {id, storer: invoker});
        res.json(capability);
      } else {
        const query = {storer: database.hash(invoker)};
        const results = await brZCapStorage.delegated.find(
  ...
  app.delete(
    ...
    asyncHandler(async (req, res) => {
      ...
      const {id} = req.query;
      const removed = await brZCapStorage.delegated.remove(
        {storer: invoker, id});
      if(removed) {
        res.status(204).end();
      } else {
...
async function _verifyDelegation({dataHubId, storer, capability}) {
