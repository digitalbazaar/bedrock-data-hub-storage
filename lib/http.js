...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  // TODO: endpoints for documents and everything else will use HTTP signatures
  // that rely on invoking ocaps
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      // TODO: run http signature + ocapld check, where invoker and delegator
      // for the document are provided as those in the data hub config
  ...
  app.post(
    ...
    validate('bedrock-data-hub-storage.document'),
    asyncHandler(async (req, res) => {
      // TODO: run http signature + ocapld check, where a document loader
      // for zcaps will handle a request for the current URL by dynamically
      // creating a zcap with security context and id... and `controller`,
      // `invoker`, and `delegator` from the data hub config
...
// TODO: call when a capability is stored
async function _verifyDelegation({capability}) {
...
async function _authorize({dataHubId, req, expectedTarget, expectedAction}) {
  // FIXME: wrap entire function in try/catch and throw not authorized?

  let capability = req.get('authorization-capability');
  const capabilityAction = req.get('authorization-capability-action');

  // TODO: wrap `parseRequest` in try/catch?
  // TODO: check http signature
  const headers = [
    '(key-id)', '(created)', '(expires)', '(request-target)',
    'host', 'authorization-capability'];
  if(capabilityAction) {
    headers.push('authorization-capability-action');
  }
  ...
  const parsed = httpSignatureHeader.parseRequest(req, {headers});
  ...
  // fetch verification method from `keyId`
  const verificationMethod = await _getVerificationMethod(keyId);

  // FIXME: try/catch to throw custom error when key type is not supported?
  ...
  // dereference the capability if it doesn't match the current URL (i.e., it
  // is not the root capability); it must exist in local zcap storage as a
  // delegated capability
  const {baseUri} = config.server;
  const currentUrl = `${baseUri}${req.originalUrl}`;
  if(capability !== currentUrl) {
    // TODO: use new bedrock-zcap-storage
    // TODO: handle not found
    //capability = await storage.getCapability({actor: null, id: capability});
    _throwNotAllowed();
  }
