...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  const cfg = config['private-remote-storage'];
  const {routes} = cfg;
  ...
  app.put(
    routes.masterKey,
    ensureAuthenticated,
    validate('bedrock-private-remote-storage.masterKey'),
  ...
  app.post(
    routes.documents,
    ensureAuthenticated,
    validate('bedrock-private-remote-storage.encryptedDocument'),
    asyncHandler(async (req, res) => {
      const {accountId} = req.params;
      const {actor} = (req.user || {});
      const {doc} = await storage.insert({actor, accountId, doc: req.body});
      res.status(201).location(baseUri + '/' + doc.id).end();
    }));
