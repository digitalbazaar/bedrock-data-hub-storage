...
const DOCUMENT_LOADER = async url => {
  ...
  if(url.startsWith('did:key:')) {
  ...
  } else if(url.startsWith('did:v1')) {
    const [did, hashFragment] = url.split('#');
    if(hashFragment) {
      if(cache[url]) {
        return cache[url];
      }
    ...
    // Get mode and hostname from config
    const {mode} = bedrock.config['data-hub-storage']['veres-one'];
    const {hostname} = bedrock.config['data-hub-storage']['veres-one'];
    const httpsAgent = new https.Agent({
      rejectUnauthorized: !bedrock.config['data-hub-storage'].ignoreSslErrors
    });
    const driverOptions = {mode, httpsAgent, hostname};
    const veresDriver = v1.driver(driverOptions);
    const {doc: didDoc} = await veresDriver.get({did});
    const document = await addKeyAgreementKey({didDoc});
    if(hashFragment) {
      ...
      const {documentLoader} = v1;
      const key = await jsonld.compact(subGraph, context, {documentLoader});
      // Handle the case when the Key Type is not defined in the context which
      // results in a prepended '/' character to the type
      // An example...
      // "type": "/X25519KeyAgreementKey2019",
      if(key.type.includes('/')) {
        key.type = key.type.replace(/\//, '');
      }
      cache[url] = {
        contextUrl: null,
        documentUrl: url,
...
async function addKeyAgreementKey({didDoc}) {
  const kak = await getKeyAgreementKey({didDoc});
  return {
    ...didDoc,
    keyAgreement: [{
      id: kak.id,
      type: kak.type,
      controller: kak.controller,
      publicKeyBase58: kak.publicKeyBase58
    }]
  };
}

async function getKeyAgreementKey({didDoc}) {
  const [authenticationKey] = didDoc.authentication;
  const edKeyPair = await LDKeyPair.generate(authenticationKey);
  return createKeyAgreementKey({edKeyPair});
}

function createKeyAgreementKey({edKeyPair}) {
  const kak = X25519KeyPair.fromEdKeyPair(edKeyPair);
  kak.id = `${edKeyPair.controller}#${kak.fingerprint()}`;
  return kak;
}
