...
bedrock.events.on('bedrock-mongodb.ready', async () => {
  await promisify(database.openCollections)(['dataHub', 'dataHubKey']);
...
api.insertKey = brCallbackify(async ({actor, accountId, key}) => {
  ...
  try {
    const result = await database.collections.dataHubKey.insert(
...
api.getKey = brCallbackify(async ({actor, accountId}) => {
  ...
  const record = await database.collections.dataHubKey.findOne(
    {accountId: database.hash(accountId)},
    {_id: 0, doc: 1, meta: 1});
  if(!record) {
    throw new BedrockError(
...
api.insert = brCallbackify(async ({actor, accountId, doc}) => {
  ...
  try {
    const result = await database.collections.dataHub.insert(
      record, database.writeOptions);
    return result.ops[0];
  } catch(e) {
    if(!database.isDuplicateError(e)) {
...
api.find = brCallbackify(async (
  ...
  return database.collections.dataHub.find(
...
api.update = brCallbackify(async ({actor, accountId, doc}) => {
  ...
  await database.collections.dataHub.update({
...
api.remove = brCallbackify(async ({actor, accountId, id}) => {
  ...
  const result = await database.collections.dataHub.remove(
    {accountId: database.hash(accountId), id: database.hash(id)});
